...
- **RenderTester** (`render_tester.py`) — the only gate that executes the code: `compile()` for syntax, then `importlib` module execution in a thread (default 60 s timeout, `RENDER_TEST_TIMEOUT_SECONDS`) to catch `NameError`/`AttributeError`/`TypeError`/LaTeX errors with parsed line numbers and fix suggestions. Note this is import-time execution only — `construct()` isn't run, so invalid Manim API calls inside it surface at render time instead.
...
backend/
  ...
  agents/
    ...
    render_tester.py         Gate 4: import-time execution test
