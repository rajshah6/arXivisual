...
3. rolls `arxivisual-api` and waits until its newest revision carries the image and reads `Healthy` / `Running` — `az containerapp update` returns before the revision is provisioned, and in single-revision mode the old revision keeps answering until the new one passes its probes, so neither the CLI's exit code nor a plain 200 proves anything. It then polls `GET /api/health` until its `commit` equals the sha;
...
> **What `commit` can say.** `GET /api/health` reports the sha baked in at build time ([backend/api/health.py](../backend/api/health.py)). An image built without `--build-arg APP_COMMIT_SHA` answers `"commit": "unknown"`, which never satisfies the poll (the run fails after ~10 minutes — pass the build arg when building by hand). An image from before the key existed answers with no `commit` at all; for that one case the workflow warns instead of failing, and the roll is proven at the revision level only, exactly like the worker's.
...
The health path is `GET /api/health` — there is no `/health` (it answers 404). It reports database, Manim, and storage connectivity; expect `"status": "healthy"` with `"database": "connected"`, `"manim": "available (...)"`, and `"storage": "r2: connected"`. Its top-level `"commit"` must equal the sha you deployed (see the note in [section 1](#1-deploy-with-the-workflow)) and `"database_dialect"` must read `postgresql` (in production the API refuses to start on anything else, rather than falling back to an ephemeral SQLite file). The worker has no URL, so the proof of its roll is the revision table: each app must show a single active revision, on the tag you deployed, `Healthy` and `Running` — and the same tag on both. Also confirm `POST /api/render` returns 404 (see `RENDER_API_SECRET` below).
