...
| GET | `/api/health` | Database / Manim / storage health (the only health path — there is no `/health`), plus the `commit` baked into the image and the `database_dialect`; the checks are cached for ~20 s and run off the event loop |
