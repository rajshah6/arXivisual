...
- **The pipeline executes LLM-generated Python on the server.** Every animation is Manim code written by a model from the text of an arXiv paper, so paper content is untrusted input to a code generator. That code runs twice, both times in a subprocess rather than in the API or worker process: once in the dry-run validation gate and once in the real render. The two are not equally contained (as of 2026-09-18). The dry-run gate strips the child's environment of the `AZURE_*`, `S3_*`, `LANGFUSE_*` and `DEDALUS_*` variables, `DATABASE_URL`, `RENDER_API_SECRET` and `OPENAI_API_KEY` (replaced by a placeholder) (`backend/agents/render_tester.py`); it does not yet strip the Application Insights connection string, the PostHog key or, on the API's in-process fallback path, the Turnstile and IP-hash secrets. The real render **inherits the full environment of the process that started it** (`backend/rendering/local_runner.py`): narration synthesis needs the Azure OpenAI key there, and nothing else is removed yet. Neither is a sandbox — there is no syscall or network isolation beyond the container. A way to make generated code read a secret, reach the database or the video bucket, or persist beyond its render is squarely in scope.
